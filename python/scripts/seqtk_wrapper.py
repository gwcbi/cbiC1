...
echo "[---$SN---] ($(date))  fastx executable: $(which fastq_quality_filter)"
...
def main(args):
    ...
    for i,node in enumerate(nodes):
        ...
        if args.nosubmit:
            with open('seqtk_job.%02d.sh' % (i+1), 'w') as outh: 
                print >>outh, '\n'.join(job_sh)
            print >>sys.stderr, '[--- job: seqtk%02d ---] Slurm script written to "seqtk_job.%02d.sh".' % ((i+1), (i+1))
        else:
            # Submit
...
if __name__ == '__main__':
    ...
    slurm_group.add_argument('--njobs', type=int,
                             help='''Total number of SLURM jobs to start. Each job will use
                                     one node.''')
    slurm_group.add_argument('--nslots', type=int, default=4, 
                             help='''Number of simultaneous processes that can run on a
                                     node. For this wrapper, each process requires a
